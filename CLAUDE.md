...
- macOS (Apple Silicon) support is fully self-contained too: the hold shortcut polls CGEventSourceKeyState over dart:ffi (same edge-detection model as Windows/Linux), typing posts CGEventPost unicode keyboard events (20 UTF-16 units per event, flags zeroed so held modifiers cannot corrupt the text), and the Swift runner owns the NSPanel listening overlay, SMAppService login item, the launch-time Accessibility prompt, and the quit flow (Dart stops the resident servers, then exits) behind the `typemate/macos` method channel. Typing requires the Accessibility permission; `isGlobalShortcutAvailable` reports it honestly. Speech runtimes bundle into `Contents/Resources` (data files inside `Contents/MacOS` invalidate the code signature) and the STT search path adds `<executable dir>/../Resources` on macOS. whisper.cpp v1.9.1 is built by `scripts/build-whisper-macos.sh` (static, Metal, embedded metallib) and hosted on the models-v1 release; sherpa uses the upstream `osx-arm64-static-no-tts` archive. `scripts/package-macos-release.sh` builds, bundles, ad-hoc signs, and archives the .app. GOTCHA: path_provider's macOS temp directory does not exist on a fresh install and flutter_single_instance then misreports a second instance and exits at launch; main.dart creates the directory first. The app is not sandboxed (CGEventPost, spawning bundled servers, and login items all require it); mic access needs `NSMicrophoneUsageDescription` plus the `audio-input` entitlement. A bundled .app launches with `/` as the working directory, so recordings go under the TypeMate data directory, not a relative path.
