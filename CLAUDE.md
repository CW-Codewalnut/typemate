...
- There are NO speech binaries anymore: every engine runs in-process. English (Parakeet) and the GTCRN noise-suppression denoiser run through the sherpa_onnx plugin; Hindi, Hinglish, and Tamil run through the whisper_ggml plugin (whisper.cpp v1.9.1, AVX2 baseline), consumed as a pinned git dependency on our fork `CW-Codewalnut/whisper_ggml`, branch `typemate-consumer` (= the upstream-facing `feat/silero-vad-passthrough` plus MSVC vendored-warning suppression kept out of the upstream PR): upstream v2.6.0 (which shipped our proposed resident-model API — `keep_model_loaded` + `releaseModel`, checkout semantics, all platforms) plus our Silero VAD passthrough across all THREE native shims (android/, ios/Classes/, macos/Classes/ — always sweep for every handler copy), offered upstream as sk3llo/whisper_ggml#28; repoint to pub.dev when it merges and releases. Known upstream gap: the macOS shim predates 2.6.0's resident model, so macOS reloads per request until upstream syncs it. `bin/` exists only on Linux, for ffmpeg (capture) and xdotool (typing) — tools, not speech engines. The LARGE models are NOT shipped in release installers: `scripts/slim-speech-models.sh` strips them from every artifact, and the app downloads the selected language's model on first use into `<data dir>/models/` (size + SHA-256 verified against `lib/src/core/stt/speech_model_catalog.dart` before a file can look complete — a corrupt model aborts the native loader, so nothing unverified may reach it). A bundled copy always wins over downloading, so dev checkouts and `flutter run` (which fetch everything via `tool/fetch_whisper_runtime.dart`) never download at runtime. Small always-needed models stay bundled: Silero VAD and GTCRN.
- English runs IN-PROCESS: `LanguageRoutingSttEngine` routes `en` to its own `SherpaParakeetSttEngine` loading NVIDIA parakeet-unified-en-0.6b int8 (non-streaming export; NVIDIA Open Model License — commercial use and redistribution permitted), and the 24 other Parakeet languages to a second `SherpaParakeetSttEngine` on Parakeet TDT 0.6B v3 int8 — both through the sherpa_onnx FFI plugin in long-lived isolates, the same engines Android uses, 4 decode threads on desktop. The split exists because real-recording benchmarking (2026-08, `test_assets/stt_benchmark/`) showed v3 degrading on accented English while unified beat every candidate on all seven accent clusters (both re-measured on the same 101 clips: 4.4% vs v3's 9.0% WER overall; Indian English 7.0% vs 11.7%; US English 4.8% vs 11.6%. Re-run BOTH models whenever the corpus grows, or the two sides stop being the same audio); each model provisions independently, so an English-only user never downloads v3. The unified family also has streaming exports if live dictation preview ever lands. No server process, no port, no startup handshake; a load failure surfaces as a real exception instead of a connection timeout (this replaced the retired sherpa websocket server and eliminated its "server did not start"/60s-timeout failure class seen in Sentry). Per-utterance English is ~1s.
...
- The language picker is curated (`lib/src/models/speech_language_options.dart`): the 25 Parakeet languages (English on parakeet-unified; 24 European on v3 with automatic language detection) plus Hindi, Hinglish, and Tamil. Do not add a language without a validated dedicated model (quality and latency) first.
- Use `tool/transcribe_directory.dart --manifest test_assets/stt_benchmark/manifest.json` to score any sherpa-onnx model with per-dialect WER against the persistent corpus: synthetic TTS clips plus real human recordings (own phone recordings, LibriSpeech, OpenSLR SLR83, Common Voice India, People's Speech dirty, AMI meetings, Speech Accent Archive) organized as `<language> (<code>)/` folders, English subdivided into seven accent-cluster folders. `manifest.json` is the database: every clip carries its reference transcript, source, license, and a `verified` flag (dataset ground truth vs consensus draft). Real/dataset audio is gitignored (personal recordings + non-redistributable licenses); only synthetic clips are committed. Pass `--language` for the clips you want; it also sets the decode language for multilingual whisper exports (forcing `en` on other-language audio measures nothing). `tool/benchmark_stt_corpus.dart` remains for the whisper-CLI path.
...
- Avoid unnecessary complicated code. Keep the code simple and easy to understand, readable and maintainable.
- Be consistent with rest of the code.
