...
- Hindi and Hinglish stay on spawn-per-utterance whisper-cli (benchmarked on the target i5-11300H laptop): Hindi uses `models/ggml-small-vaani-hindi-q6.bin` (Vaani small fine-tune, ~2.8s, noise-robust), Hinglish uses `models/ggml-hindi2hinglish-swift.bin` (Oriserve Swift fine-tune, base-sized, ~1.2s, writes romanized Hinglish; our own GGML conversion hosted on this repo's GitHub releases). A resident server would not help them: their latency is encoder compute, not model loading.
